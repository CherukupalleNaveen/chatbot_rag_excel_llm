...
def query_rag(query):
    ...
    docs = [m.get("text", "") for m in results["metadatas"][0]]
    context = "\n".join(docs)

    prompt = f"""You are a helpful assistant for Chalapathi College Admissions.
    Use the following information to answer the user's query politely and concisely.
